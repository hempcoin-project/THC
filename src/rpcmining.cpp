...
{
    return (uint64_t)GetProofOfWorkReward(GetLastBlockIndex(pindexBest, false)->nHeight, 0, 0);
}
...
Value getmininginfo(const Array& params, bool fHelp)
{
    ...
    obj.push_back(Pair("blockvalue",    (uint64_t)GetProofOfWorkReward(GetLastBlockIndex(pindexBest, false)->nHeight, 0, 0)));
...
Value getworkex(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "Hempcoin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "Hempcoin is downloading blocks...");
