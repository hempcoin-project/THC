...
int64_t GetProofOfWorkReward(int nHeight, int64_t nFees, uint256 prevHash)
{
	...
	else if (nHeight < 200001) {nSubsidy = 250 * COIN;}
...
unsigned int GetNextTargetRequired(const CBlockIndex* pindexLast, bool fProofOfStake)
{
	...
	if(pindexLast->nHeight > HEMPCOIN_SWITCHOVER_BLOCK)
	{
		if(nActualSpacing < 0)
		{
			// printf(">> nActualSpacing = %"PRId64" corrected to 2.\n", nActualSpacing);
			nActualSpacing = 2;
		}
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in Hempcoin
    bool fStrictPayToScriptHash = true; // Always active in Hempcoin
...
bool CBlock::AcceptBlock()
{
    ...
    /*
    disable sync-checkpoints because they consume way too much CPU time
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Hempcoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "Inhale, exhale, just got an oz in the mail...";
        CTransaction txNew;
        txNew.nTime = 1393868740;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
	...
	if(fTestNet)
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0x2362d9763557a78a460e62d6c0439aa6fcf15ca607746e56f1026d922302c0ad"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xa5, 0xa5, 0xfd, 0x01 };
