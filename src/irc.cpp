...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #hempcoinTEST2\r");
            Send(hSocket, "WHO #hempcoinTEST2\r");
        } else {
            // randomly join #hempcoin00-#hempcoin05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #hempcoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #hempcoin%02d\r", channel_number).c_str());
        }
